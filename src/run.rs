...
pub fn run(
    ...
    loop {
        ...
        if !event::poll(
            ...
            if let Ok(guard) = SHARED_MENU_INJECTION.lock() {
            ...
            let is_ai_active = SHARED_AI_PROGRESS.lock().map(|p| p.active).unwrap_or(false);
            if MessageBar::is_mpv_render(&framework.split_clean().0) || is_ai_active {
            ...
            Ok(evt) => match evt {
                Event::Mouse(mouse)
                    if framework
                        ...
                        .mouse_support =>
                {
                    log(&format!("MOUSE EVENT: {:?}", mouse.kind));
                            ...
                            updated = updated
                                || match scroll_behaviour {
                                    TextbarScrollBehaviour::Character => {
                                        command_capture.as_mut().map_or(false, |c| c.left().is_ok())
                                    }
                                    TextbarScrollBehaviour::History => {
                                        let status = framework.data.global.get::<Status>().unwrap();
                                        if let Some(history) = framework.data.global.get::<CommandHistory>() {
                                                ...
                                                && !history.0.is_empty()
                                            {
                                                ...
                                                if let Some(text_field) = status.command_capture.as_mut() {
                                    ...
                                    TextbarScrollBehaviour::Word => {
                                        if let Some(text_field) = framework
                                            .data
                                            .global
                                            .get_mut::<Status>()
                                            .and_then(|s| s.command_capture.as_mut())
                                        {
                                            previous_word(text_field);
                                            true
                                        } else { false }
                                    }
                                }
                        ...
                        MouseEventKind::ScrollDown if command_capture.is_some() => {
                            updated = updated
                                || match scroll_behaviour {
                                    ...
                                    TextbarScrollBehaviour::Character => {
                                        command_capture.as_mut().map_or(false, |c| c.right().is_ok())
                                    }
                                }
                        }
